...
images= cv2.imread('source/test1.jpg')
...
while True:
    ...
    imgblur = cv2.GaussianBlur(mask,(5,5),1)
    imgcanny = cv2.Canny(imgblur,100,250)
    kernel = np.ones((5, 5), np.uint8)
    dilate = cv2.dilate(imgcanny, kernel,iterations=2)
    erode = cv2.erode(imgcanny,kernel,iterations=1)
